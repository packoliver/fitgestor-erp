-- Achado da auditoria de ciclo completo (pedido do usuário: "cada ação tem
-- que ligar uma coisa na outra pra não dar furo"): cancel_sale (estorno de
-- venda) já reverte estoque, crédito de loja, vale-troca e cash_movements,
-- mas nunca foi ligado ao contas a receber de cartão (card_receivables,
-- criado no item 7 da auditoria do PDV). Resultado: estornar uma venda paga
-- no cartão deixava a(s) parcela(s) pendente(s) em card_receivables como se
-- o dinheiro ainda fosse cair — nunca seriam recebidas de verdade, mas
-- também nunca seriam baixadas/canceladas.
--
-- Esta migration:
-- 1. Permite o status 'cancelled' em card_receivables.
-- 2. Bloqueia o estorno se alguma parcela já foi marcada como recebida ou
--    conciliada (dinheiro real já contabilizado — exige decisão manual
--    antes, não pode ser desfeito silenciosamente).
-- 3. Cancela automaticamente as parcelas ainda pendentes da venda.

ALTER TABLE public.card_receivables DROP CONSTRAINT IF EXISTS card_receivables_status_check;
ALTER TABLE public.card_receivables
  ADD CONSTRAINT card_receivables_status_check
  CHECK (status = ANY (ARRAY['pending'::text, 'received'::text, 'reconciled'::text, 'cancelled'::text]));
