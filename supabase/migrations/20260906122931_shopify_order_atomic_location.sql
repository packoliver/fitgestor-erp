DROP FUNCTION IF EXISTS public.import_shopify_order_atomic(uuid, jsonb);
...
BEGIN
  ...
  FOR v_row IN
    SELECT i.variant_id, sum(i.quantity)::integer AS quantity
     ...
     ORDER BY i.variant_id
  LOOP
    PERFORM public.apply_stock_movement_system(
      _organization_id,
      v_row.variant_id,
      v_location_id,
      'venda',
      v_row.quantity,
      'Venda Shopify #' || v_order_number,
      'sale',
      v_sale_id,
      NULL,
      jsonb_build_object('source', 'shopify', 'external_order_id', v_external_id)
    );
  END LOOP;
