-- Restaura a versão completa de cancel_sale após uma definição histórica
-- mais antiga ter sobrescrito proteções já aplicadas em migrations posteriores.
-- Mantém validações de expedição, recebíveis, caixa, crédito e vale-troca e
-- acrescenta uma ordem determinística de locks de estoque para evitar deadlocks.
...
BEGIN
  ...
  -- Trava saldos sempre na mesma ordem para impedir deadlocks quando duas
  -- transações concorrentes envolvem as mesmas variações em ordem diferente.
  FOR v_item IN
    SELECT *
