...
const productSchema = z.object({
  ...
  origin: z.string().trim().max(10).optional().or(z.literal("")),
...
export function ProductForm({
  ...
  const queueShopifySync = useServerFn(queueShopifyProductSyncFn);
  const getShopifySyncStatus = useServerFn(getShopifyProductSyncStatusFn);
  const [values, setValues] = useState<ProductFormValues>({
    ...
    seo_keywords: Array.isArray(initial?.seo_keywords) ? initial.seo_keywords.join(", ") : "",
    seo_description: initial?.seo_description ?? "",
    status: (initial?.status as any) ?? "ativo",
    shopify_publish: initial?.shopify_publish ?? false,
  });
  ...
  const [variants, setVariants] = useState<VariantInput[]>(
    initialVariants.length > 0
      ? initialVariants.map((v) => ({
          ...
          promotional_price: v.promotional_price?.toString() ?? "",
          initial_stock: "",
          minimum_stock: "",
        }))
  ...
  const shopifySync = useQuery({
    queryKey: ["shopify-product-sync", productId],
    enabled: Boolean(productId),
    queryFn: () => getShopifySyncStatus({ data: { productId: productId! } }),
    refetchInterval: (query) => {
      const status = (query.state.data as any)?.job?.status;
      return status === "pending" || status === "processing" || status === "retry" ? 5000 : false;
    },
  });
  ...
  function handleGenerateRowSKUAndEAN(index: number) {
    setVariants((prev) =>
      prev.map((v, i) => {
        ...
        const newSku =
          v.sku.trim() ||
          generateSKU(values.name, (v.color.trim() || values.color) ?? undefined, v.size);
  ...
  function handleSelectFiles(files: FileList | null) {
    ...
    if (productId) {
      ...
      toast.info(
        `${newPendings.length} foto(s) adicionada(s) à prévia local. Elas serão salvas ao criar o produto.`,
      );
    }
  }
  ...
  async function uploadPendingFiles(
    ...
    itemsToUpload: { file: File; preview: string }[],
  ) {
  ...
  async function handleSaveInlineItem() {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({ name: newItemName.trim(), organization_id: org })
        .select("id, name")
        .single();
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      for (const variant of variants) {
        const normal = variant.sale_price.trim()
          ? Number(variant.sale_price.replace(",", "."))
          : null;
        const promo = variant.promotional_price.trim()
          ? Number(variant.promotional_price.replace(",", "."))
          : null;
        if (
          (normal !== null && (!Number.isFinite(normal) || normal < 0)) ||
          (promo !== null &&
            (!Number.isFinite(promo) || promo <= 0 || (normal !== null && promo > normal)))
        ) {
          throw new Error(`Preço ou promoção inválida na variação ${variant.size}.`);
        }
      }
      const org = await currentOrgId();
      if (!org) throw new Error("Organização não encontrada");
      ...
      const payload = {
        ...
        promotional_price: values.promotional_price
          ? Number(values.promotional_price.replace(",", "."))
          : null,
        ncm: parsed.data.ncm?.trim() || null,
        origin: parsed.data.origin?.trim() || null,
        cest: parsed.data.cest?.trim() || null,
        ...
        shopify_publish: parsed.data.shopify_publish,
      };
      ...
      for (const v of variants) {
        ...
        if (sku && variants.filter((x) => x.sku.trim() === sku).length > 1)
          throw new Error(`SKU duplicado no formulário: ${sku}`);
        if (barcode && variants.filter((x) => x.barcode.trim() === barcode).length > 1)
          throw new Error(`Código de barras duplicado no formulário: ${barcode}`);
        ...
        const varPayload = {
          ...
          promotional_price: v.promotional_price
            ? Number(v.promotional_price.replace(",", "."))
            : null,
        };
        if (v.id) {
      ...
      const sync = await queueShopifySync({ data: { productId: id! } });
      return { id: id!, sync };
    },
    onSuccess: ({ id, sync }) => {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-3">
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2 space-y-2">
              ...
              <Input
                ...
                placeholder="Ex.: Legging Dry Fit High Waist"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => setValues({ ...values, category_id: v })}
              >
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => setValues({ ...values, brand_id: v })}
              >
                <SelectTrigger>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  size="sm"
                  className="h-5 px-1 text-xs text-primary"
                  onClick={() => setInlineModal("supplier")}
                >
                  <PlusCircle className="mr-1 h-3 w-3" />
                  Novo
                </Button>
              ...
              <Select
                value={values.supplier_id ?? undefined}
                onValueChange={(v) => setValues({ ...values, supplier_id: v })}
              >
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setValues({ ...values, collection: e.target.value })}
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Ex.: Poliamida / Elastano"
              />
            </div>
            <div className="sm:col-span-2 space-y-2">
              ...
              <Input
                ...
                placeholder="Resumo para catálogo / site"
              />
            </div>
            <div className="sm:col-span-2 space-y-2">
              ...
              <Textarea
                ...
                placeholder="Detalhes técnicos, tabela de medidas e cuidados com a peça"
              />
            </div>
            <div className="space-y-2">
              <Label>NCM</Label>
              <Input
                value={values.ncm ?? ""}
                onChange={(e) => setValues({ ...values, ncm: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              <Label>Origem</Label>
            ...
            <div className="space-y-2">
              ...
              <Input
                value={values.cest ?? ""}
                onChange={(e) => setValues({ ...values, cest: e.target.value })}
              />
            </div>
            <div className="sm:col-span-2 space-y-2">
            ...
            <div className="sm:col-span-2 space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setValues({ ...values, seo_keywords: e.target.value })}
                placeholder="fitness, bermuda, suplex"
              />
            </div>
            <div className="sm:col-span-2 space-y-2">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={handleGenerateAllSKUAndEAN}
              className="text-xs"
            >
              <Wand2 className="mr-1.5 h-3.5 w-3.5" />
              Gerar SKUs e EANs para todos
            </Button>
          </CardHeader>
          <CardContent className="pt-4">
            <div className="mb-3 flex flex-wrap gap-1 items-center">
              <span className="text-xs text-muted-foreground mr-2">
                Adicionar tamanho rapidamente:
              </span>
                  ...
                  onClick={() =>
                    setVariants((prev) => {
                      ...
                      const exists = prev.some(
                        (v) => v.size === s && v.color.trim().toLowerCase() === color.toLowerCase(),
                      );
            ...
            <div className="space-y-2">
              {variants.map((v, i) => (
                <div
                  key={i}
                  className="grid gap-2 rounded-lg border p-3 sm:grid-cols-12 items-center"
                >
                  ...
                  <div className="sm:col-span-2">
                    <Input
                      aria-label={`Preço normal ${v.size}`}
                      placeholder="Preço normal"
                      value={v.sale_price}
                      onChange={(e) => updateVariant(i, "sale_price", e.target.value)}
                    />
                    <Input
                      aria-label={`Preço promocional ${v.size}`}
                      placeholder="Promoção"
                      value={v.promotional_price}
                      onChange={(e) => updateVariant(i, "promotional_price", e.target.value)}
                    />
                    {!v.id ? (
                      <Input
                        placeholder="Estoque"
                  ...
                  <div className="sm:col-span-1 flex items-center justify-end gap-1">
                    <Button
                      ...
                      variant="ghost"
                      size="icon"
                      title="Gerar SKU e EAN para esta variação"
                      onClick={() => handleGenerateRowSKUAndEAN(i)}
                    >
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  setVariants([...variants, emptyVariant("", (values.color ?? "").trim())])
                }
              >
      ...
      <div className="space-y-4">
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-start justify-between gap-4">
              <div>
                <Label htmlFor="shopify-publish">Publicar na vitrine Shopify</Label>
                <p className="mt-1 text-xs text-muted-foreground">
                  Desligado: o cadastro continua no ERP e na Shopify, mas não aparece no site.
                </p>
              </div>
              <Switch
                id="shopify-publish"
                checked={values.shopify_publish}
                onCheckedChange={(checked) => setValues({ ...values, shopify_publish: checked })}
              />
            </div>
            {productId && shopifySync.data ? (
              <div className="rounded-md border bg-muted/40 p-3 text-xs">
                <div className="flex items-center justify-between gap-2">
                  <span className="font-medium">Sincronização</span>
                  <Badge
                    variant={
                      (shopifySync.data as any).job?.status === "failed"
                        ? "destructive"
                        : "secondary"
                    }
                  >
                    {shopifySyncLabel(shopifySync.data as any)}
                  </Badge>
                </div>
                {(shopifySync.data as any).product?.shopify_last_synced_at ? (
                ...
                {(shopifySync.data as any).product?.shopify_last_sync_error ? (
                  <p className="mt-2 flex gap-1 text-destructive">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setValues({ ...values, promotional_price: e.target.value })}
                placeholder="0.00"
              />
            </div>
            <div className="rounded-md bg-muted p-3 text-sm">
              Margem estimada:{" "}
              <span className="font-bold text-primary">{margin ? `${margin}%` : "—"}</span>
              {values.sale_price && (
                <div className="text-xs text-muted-foreground mt-1">
                  Venda: {formatBRL(Number(values.sale_price.replace(",", ".")))}
                </div>
              )}
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Galeria de Fotos (Supabase Storage)</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-3 gap-2">
              {images
                ...
                .map((img) => {
                  ...
                  return (
                    <div
                      key={img.id}
                      className="group relative aspect-square rounded-md overflow-hidden border bg-muted flex items-center justify-center"
                    >
                      {!isBroken ? (
                        <img
                          src={img.image_url}
                          ...
                          onError={() => {
                            ...
                            console.warn(
                              `[Supabase Storage] Imagem ${img.id} não pôde ser carregada (${img.image_url}). Verifique se o bucket 'product-images' está PÚBLICO no Supabase.`,
                            );
                          }}
                        />
                      ) : (
              ...
              {pendingFiles.map((p, idx) => (
                <div
                  key={idx}
                  className="group relative aspect-square rounded-md overflow-hidden border border-dashed border-primary bg-primary/5"
                >
                  <img src={p.preview} alt="Prévia Local" className="h-full w-full object-cover" />
                  <Badge
                    variant="secondary"
                    className="absolute top-1 left-1 text-[9px] bg-background/90 shadow-sm"
                  >
              ...
              {uploading ? (
                ...
                <>
                  <div className="flex items-center gap-2 text-sm font-medium">
                    <Upload className="h-4 w-4 text-primary" /> Selecionar imagens
                  </div>
                  <span className="text-[11px] text-muted-foreground mt-1">
                    Prévia instantânea via URL.createObjectURL
                  </span>
                </>
              )}
              <input
      ...
      <Dialog open={!!inlineModal} onOpenChange={(o) => !o && setInlineModal(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setInlineModal(null)}>
              Cancelar
            </Button>
...
function shopifySyncLabel(data: any) {
  if (!data?.runtime?.enabled) return "Desativada com segurança";
  const status = data?.job?.status;
  if (status === "completed") return "Sincronizado";
  if (status === "processing") return "Sincronizando";
  if (status === "retry") return "Nova tentativa agendada";
  if (status === "failed") return "Falha — revisar";
