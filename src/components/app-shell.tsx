...
import { getOpenSession } from "@/lib/pos";
import {
  NAV_ITEMS,
  itemsByGroup,
  filterByPermission,
...
function applyCourierFilter(items: NavItem[], has: (c: string) => boolean) {
  return items.filter((i) => {
    ...
    return (
      has("shipping.view_own") && !has("shipping.view_all") && !has("shipping.manage_couriers")
    );
  });
}
...
function AppSidebar({
  ...
  useEffect(() => {
    if (typeof window !== "undefined")
      window.localStorage.setItem(LS_ESSENTIAL, essential ? "1" : "0");
  }, [essential]);
        ...
        >
          ...
          {!collapsed && (
            <>
              ...
              <kbd className="hidden md:inline-flex h-5 items-center rounded-md border border-slate-200 bg-white px-1.5 text-[10px] font-mono font-bold text-slate-500">
                ⌘K
              </kbd>
            </>
          )}
              ...
              {NAV_GROUPS.map((g) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Meta.icon
                      className={`h-[18px] w-[18px] ${isSel ? "text-blue-600" : "text-slate-400"}`}
                    />
        ...
        {!collapsed ? (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                <Avatar className="h-7.5 w-7.5 shrink-0">
                  <AvatarFallback className="bg-blue-100 text-blue-700 text-xs font-extrabold">
                    {initials}
                  </AvatarFallback>
                </Avatar>
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-400 font-normal truncate leading-tight">
                    FitGestor ERP
                  </p>
                </div>
            ...
            <DropdownMenuContent
              side="top"
              align="start"
              className="w-56 rounded-xl border border-slate-200 shadow-md bg-white"
            >
...
export function AppShell({ children, userEmail }: { children: ReactNode; userEmail: string }) {
  ...
  const initials =
    userEmail && typeof userEmail === "string" ? userEmail.slice(0, 2).toUpperCase() : "FG";
  const showFloatingReopen = !isMobile && !pinned && !sidebarOpen;

  // O status do caixa vem da sessão real no Supabase. Nunca inferir abertura
  // por localStorage nem usar um valor inicial fictício.
  const { data: openCashSession } = useQuery({
    queryKey: ["current-session"],
    queryFn: () => getOpenSession(),
    staleTime: 2_000,
    refetchInterval: 5_000,
  });
  const cashShiftStatus: "open" | "closed" = openCashSession ? "open" : "closed";
  const cashShiftInitial = Number(openCashSession?.opening_amount ?? 0);
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="h-16 flex items-center gap-3 px-4 sm:px-6 sticky top-0 z-20 bg-white/95 dark:bg-zinc-900/95 backdrop-blur border-b border-slate-200/80 dark:border-zinc-800 shadow-xs">
            ...
            >
              ...
              <kbd className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 hidden lg:inline-flex h-6 items-center rounded-md border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-1.5 text-[10px] font-mono font-bold text-slate-600 dark:text-slate-300">
                Ctrl+K
              </kbd>
            </button>
            ...
            <Button
              ...
              className="relative text-slate-600 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-xl"
            >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  className="h-10 gap-2 pl-1.5 pr-3 rounded-xl text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-zinc-800"
                >
                  <Avatar className="h-7 w-7">
                  ...
                  <span className="hidden sm:inline text-xs font-bold max-w-[140px] truncate">
                    {userEmail}
                  </span>
                </Button>
              </DropdownMenuTrigger>
