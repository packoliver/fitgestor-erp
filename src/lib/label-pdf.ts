...
export type LabelTemplate = {
  ...
  policy_text?: string;
  /** Etiquetas por linha da bobina. Padrão 1 (uma etiqueta por página, comportamento antigo). */
  columns?: number;
  /**
   * Distância em mm entre o início de uma coluna e o início da próxima (passo/pitch horizontal).
   * Só é usado quando `columns` > 1. Equivale ao campo "Espaçamento Horizontal" da Olist.
   */
...
/** Texto copiado do modelo de produto "ELGIN 42 / TROCA" da Olist. */
export const OLIST_ELGIN_EXCHANGE_POLICY =
  "NÃO TROCAMOS: ITENS PROMOCIONAIS, SEM ETIQUETA FIXADA OU APÓS O PRAZO!";
...
/**
 * Modelo usado anteriormente na Olist: ELGIN 42 / TROCA.
 * Bobina, retrato, 70 x 100 mm, fonte 10 pt, margens de página zeradas,
 * 2 colunas com 75 mm de espaçamento horizontal (valores conferidos direto
 * em "Configurações > etiquetas > ELGIN 42 / TROCA > editar dimensões" na Olist).
 */
export const OLIST_ELGIN_42_TEMPLATE: LabelTemplate = {
  width: 70,
  height: 100,
  margin_top: 0,
  margin_right: 0,
  ...
  show_price: true,
  layout: "qsf-standard",
  policy_text: OLIST_ELGIN_EXCHANGE_POLICY,
  columns: 2,
  column_spacing: 75,
};
...
function drawCompactLabel(
  ...
  const ml = Number(template.margin_left) + xOffset;
  const mt = Number(template.margin_top);
  const innerW = Math.max(1, w - Number(template.margin_left) - Number(template.margin_right));
...
export function generateLabelPdf(
  ...
  // Etiquetas por linha da bobina (ex: 2 no modelo Olist ELGIN 42). Padrão 1 = comportamento antigo.
  const columns = Math.max(1, Math.floor(Number(template.columns) || 1));
  // Passo horizontal entre colunas (campo "Espaçamento Horizontal" da Olist). Sem colunas extras, é a própria largura.
  const columnSpacing = columns > 1 && Number(template.column_spacing) > 0 ? Number(template.column_spacing) : w;
  const pageW = columns > 1 ? columnSpacing * (columns - 1) + w : w;

  // Instancia jsPDF com formato de página igual às dimensões exatas da etiqueta (ou da linha de etiquetas) em mm
  const pdf = new jsPDF({
    unit: "mm",
    format: [pageW, h],
    orientation: pageW >= h ? "landscape" : "portrait",
  });
  ...
  const drawLabel = (it: LabelPayload, xOffset: number) => {
    if (layout === "thermal") {
      drawThermalTagLabel(pdf, it, template, orgName, xOffset);
    } else if (layout === "qsf-standard") {
      drawQsfLabel(pdf, it, template, orgName, xOffset);
    } else {
      drawCompactLabel(pdf, it, template, orgName, xOffset);
    }
  };

  // Cada linha da bobina vira uma página, preenchida com até `columns` etiquetas lado a lado
  for (let rowStart = 0; rowStart < pages.length; rowStart += columns) {
    if (rowStart > 0) {
      pdf.addPage([pageW, h], pageW >= h ? "landscape" : "portrait");
    }
    for (let col = 0; col < columns; col++) {
      const it = pages[rowStart + col];
      if (!it) break; // última linha pode ficar incompleta
      drawLabel(it, col * columnSpacing);
    }
  }
