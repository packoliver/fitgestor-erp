...
function norm(s: unknown): string {
  return stripAccents(String(s ?? "")).trim().toLowerCase().replace(/[._-]+/g, " ").replace(/\s+/g, " ").trim();
}
 ...
 * da grade. Código/SKU nunca é tamanho. Sem tamanho informado, usa "ÚNICO".
 */
export function parseOlistVariation(v: any): ParsedVariation {
  ...
  if (!size) {
    ...
    // Only a size-shaped description is evidence of size, not arbitrary item text.
    size = /^(pp|p|m|g|gg|xg|xgg|u|un|unico|\d{1,3})$/i.test(stripAccents(desc)) ? desc : "ÚNICO";
  }
