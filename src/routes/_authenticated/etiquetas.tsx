...
import { Trash2, Printer, Settings2, Plus } from "lucide-react";
import { toast } from "sonner";
import {
  generateLabelPdf,
  QSF_DEFAULT_TEMPLATE,
  DEFAULT_EXCHANGE_POLICY,
  OLIST_ELGIN_42_TEMPLATE,
  type LabelPayload,
  type LabelTemplate,
} from "@/lib/label-pdf";
import { listLabelTemplates, deactivateLabelTemplate, type CustomLabelTemplate } from "@/lib/label-templates-repo";
import { LabelTemplateEditorDialog } from "@/components/label-template-editor-dialog";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
type PresetKey = "qsf-standard" | "olist-elgin-42" | "compact";
...
const PRESETS: Record<PresetKey, { label: string; template: LabelTemplate }> = {
  ...
  "olist-elgin-42": {
    label: "Olist — ELGIN 42 / TROCA (70 × 100 mm)",
    template: OLIST_ELGIN_42_TEMPLATE,
  },
  compact: {
...
function EtiquetasPage() {
  ...
  const [selectedId, setSelectedId] = useState<string>(() => {
    ...
    return v ?? "qsf-standard";
  });
  const [labelWidth, setLabelWidth] = useState(String(PRESETS["qsf-standard"].template.width));
  const [labelHeight, setLabelHeight] = useState(String(PRESETS["qsf-standard"].template.height));
  const [policyText, setPolicyText] = useState(DEFAULT_EXCHANGE_POLICY);
  const [editorOpen, setEditorOpen] = useState(false);
  const [editingCustom, setEditingCustom] = useState<CustomLabelTemplate | undefined>(undefined);
  const [duplicateFrom, setDuplicateFrom] = useState<{ name: string; template: LabelTemplate } | undefined>(undefined);
  const [deleteTarget, setDeleteTarget] = useState<CustomLabelTemplate | null>(null);
  ...
  const isCustomSelected = useMemo(
    () => (customTemplates.data ?? []).some((t) => t.id === selectedId),
    [customTemplates.data, selectedId],
  );

  const activeBase: { name: string; template: LabelTemplate } = useMemo(() => {
    if (isPresetKey(selectedId)) {
      return { name: PRESETS[selectedId].label, template: PRESETS[selectedId].template };
    }
    const custom = (customTemplates.data ?? []).find((t) => t.id === selectedId);
    if (custom) return { name: custom.name, template: custom.template };
    return { name: PRESETS["qsf-standard"].label, template: PRESETS["qsf-standard"].template };
  }, [selectedId, customTemplates.data]);

  useEffect(() => {
    if (typeof window !== "undefined") window.localStorage.setItem(STORAGE_KEY, selectedId);
    setLabelWidth(String(activeBase.template.width));
    setLabelHeight(String(activeBase.template.height));
    setPolicyText(activeBase.template.policy_text ?? DEFAULT_EXCHANGE_POLICY);
  ...
  const previewBlobUrl = useMemo(() => {
    ...
    const template: LabelTemplate = {
      ...activeBase.template,
      width: Number(labelWidth) || activeBase.template.width,
      height: Number(labelHeight) || activeBase.template.height,
  ...
  }, [rows, activeBase, labelWidth, labelHeight, policyText, org.data]);
  ...
  async function generatePdf() {
    ...
    await supabase.from("audit_logs").insert({
      ...
      action: "print",
      entity_type: "labels",
      new_data: { count: total, items: rows.length, template: selectedId } as any,
      user_id: userId,
  ...
  return (
    <div>
            ...
            <CardContent className="space-y-3">
              <div className="space-y-1">
                ...
                <Select value={selectedId} onValueChange={setSelectedId}>
                  ...
                  <SelectContent>
                    <SelectGroup>
                      <SelectLabel>Padrões do sistema</SelectLabel>
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  className="flex-1"
                  onClick={() => {
                    setDuplicateFrom({ name: activeBase.name, template: { ...activeBase.template, width: Number(labelWidth), height: Number(labelHeight), policy_text: policyText } });
                ...
                {isCustomSelected && (
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      const t = customTemplates.data!.find((x) => x.id === selectedId)!;
                      setEditingCustom(t);
                ...
                {isCustomSelected && (
                  <Button
                    ...
                    variant="outline"
                    size="icon"
                    onClick={() => setDeleteTarget(customTemplates.data!.find((x) => x.id === selectedId) ?? null)}
                  >
              ...
              {activeBase.template.layout === "qsf-standard" && (
                <div className="space-y-1">
      ...
      <LabelTemplateEditorDialog
        open={editorOpen}
        onOpenChange={setEditorOpen}
        editing={editingCustom}
        startFrom={duplicateFrom}
        onSaved={(id) => {
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Excluir modelo "{deleteTarget?.name}"?</AlertDialogTitle>
            <AlertDialogDescription>
              O modelo deixa de aparecer na lista. Etiquetas já impressas com ele não são afetadas.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={async () => {
                ...
                try {
                  await deactivateLabelTemplate(deleteTarget.id);
                  toast.success("Modelo excluído.");
                  queryClient.invalidateQueries({ queryKey: ["label-templates"] });
                  setSelectedId("qsf-standard");
                } catch (err: any) {
                  toast.error(err?.message ?? "Não foi possível excluir o modelo.");
                } finally {
