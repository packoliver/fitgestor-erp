...
function VendasPdvPage() {
  ...
  useEffect(() => {
    ...
    setCurrentShift({
      ...
      openedBy: session ? (sellerName || "Operador") : null,
      initialValue: Number(session?.opening_amount ?? 0),
      movements: [],
    });
  }, [session?.id, session?.opened_at, session?.opening_amount, sellerName]);
  ...
  const currentPrice = useMemo(() => {
    ...
    return Number(pickedPrice || (effectiveVariantPrice(v, v.product))) || 0;
  ...
  const complete = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      cart.forEach((l) => {
        if (l.sku) {
          pushShopifyStock({ data: { sku: l.sku } })
            .then((result) => {
              if (!result?.ok) toast.warning(result?.message ?? `Estoque do SKU ${l.sku} ficou na fila da Shopify.`);
            })
            .catch((error) => {
              console.warn(error);
              toast.warning(`Venda concluída, mas o estoque do SKU ${l.sku} ficou pendente na Shopify.`);
            });
        }
      });
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-sm text-slate-900 dark:text-slate-100 truncate">{v.product?.name}{(v.color ?? v.product?.color) ? ` — ${v.color ?? v.product?.color}` : ""}</div>
              ...
              <Card className="p-4 rounded-xl border-blue-200 bg-blue-50/40 dark:bg-blue-950/30 space-y-3 animate-in fade-in duration-150 border">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    {(pickedVariant.color ?? pickedVariant.product?.color) && <p className="text-xs text-blue-700 dark:text-blue-300 font-medium">{pickedVariant.color ?? pickedVariant.product?.color}</p>}
                  </div>
